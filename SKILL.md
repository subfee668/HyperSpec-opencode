...
**检查2：OpenSpec** — 看项目根目录是否有 `openspec/`。没有则提示：`bunx @fission-ai/openspec init`（如果 bunx 失败，改用 `npx @fission-ai/openspec init`）

注意：`openspec init` 除了创建 `openspec/` 目录外，还会在 `.claude/` 或 `.opencode/` 下生成配置文件和 slash commands。这些都是正常的副作用，不需要特别处理。
...
— 如果文件存在并且有内容读取获取数据 提示： 存在 AGENTS.md 直接读取

| 检测项 | 检测方式 | 默认值 |
|--------|---------|--------|
| **languages** | 读取AGENTS.md 从 **项目概述** 提取 | `[]` |
| **frameworks** | 读取AGENTS.md 从 **项目概述** 和 **架构要点** 提取 | `[]` |
| **build_tool** | 读取AGENTS.md 从 **构建** 提取 | `unknown` |
| **compile_command** | 读取AGENTS.md 从 **构建** 提取 | `null` |
| **test_command** | 读取AGENTS.md 从 **构建** 提取 | `null` |
| **structure** | 读取AGENTS.md 从 **架构要点** 提取 | `single-module` |
| **has_ci** | 读取AGENTS.md 从 **部署** 提取 | `false` |

**检查是否存在CLAUDE.md或claude.md 文件**

— 如果AGENTS.md不存在，但是CLAUDE.md或claude.md文件存在并且有内容读取获取数据 提示： 存在 CLAUDE.md 直接读取 

| 检测项 | 检测方式 | 默认值 |
|--------|---------|--------|
| **languages** | 读取CLAUDE.md从 **Project Overview** 提取 | `[]` |
| **frameworks** | 读取CLAUDE.md 从 **Core Framework** 提取 | `[]` |
| **build_tool** | 读取CLAUDE.md 从 **Build Commands** 提取 | `unknown` |
| **compile_command** | 读取CLAUDE.md 从 **Build Commands** 提取 | `null` |
| **test_command** | 读取CLAUDE.md 从 **测试** 提取 | `null` |
| **structure** | 读取CLAUDE.md 从 **露露结构** 提取 | `single-module` |
| **has_ci** | 读取CLAUDE.md 从 **部署** 提取 | `false` |

**如果AGENTS.md和CLAUDE.md文件都不存在**
...
Step 2（需求确认）完成后，grill-me 的决策会持久化到 `.hyperspec-requirements.md`（项目根目录），然后 Step 3 将其移入 `openspec/changes/<变更名>/requirements.md`。该文件在 propose 和 apply 阶段全程可读，确保后续步骤无需重复询问用户。

| 文件位置 | 生命周期 | 说明 |
|---------|---------|------|
| `.hyperspec-requirements.md` | Step 2 写入 → Step 3 移入后删除 | 临时文件，仅在 propose 阶段 Step 2-3 之间存在 |
| `openspec/changes/<变更名>/requirements.md` | Step 3 移入 → archive 归档 | 正式 artifact，贯穿整个变更周期 |
...
checkpoint: profiler-done | requirements-confirmed | requirements-persisted | openspec-generated | plan-generated | plan-generated-and-confirmed | task-N-complete | verified | reviewed | apply-done | consistency-verified | archived | done
project_profile:
  languages: [java]
...
**checkpoint 说明**：
- `requirements-confirmed`：grill-me 需求确认完成，但 `.hyperspec-requirements.md` 尚未写入
- `requirements-persisted`：需求摘要已持久化到 `.hyperspec-requirements.md`，可安全进入 Step 3
- 旧版状态文件中 `requirements-confirmed` 等价于新版 `requirements-persisted`（兼容处理：如果读取到 `requirements-confirmed` 且 `.hyperspec-requirements.md` 存在，视为 `requirements-persisted`）
...
- 如果有活跃变更目录但无计划文件：根据变更目录下 artifacts 的完整程度判定 checkpoint（artifacts 完整 → `openspec-generated`，不完整 → `requirements-persisted`）
- 如果 checkpoint = `requirements-confirmed`：检查 `.hyperspec-requirements.md` 是否存在 → 存在则视为 `requirements-persisted`，不存在则回退到 Step 2 补充持久化
...
| 需求摘要持久化完成 | `checkpoint: requirements-persisted`（`.hyperspec-requirements.md` 已写入） |
