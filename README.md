...
该项目是 HyperSpec 的修改版本，主要增加了 使用grill-me 和 grill-with-doc来需求澄清，并且修改适配了opencode，同时对项目初始化时直接读取AENGT.md,避免重复扫描项目。
感谢 wind7rui 的开源。


需求澄清+ 规格驱动 + 工程纪律的完整开发工作流 Skill，协调 [OpenSpec](https://github.com/fission-ai/openspec)（规格管理）和 [Superpowers](https://github.com/obra/superpowers)（TDD + 子代理审查），从需求到实现到归档一条流程走完。
...
| 依赖 | 用途 | 检查方式 | 安装方式 |
|------|------|----------|----------|
| **grill-me/grill-with-doc** CLI | 需求澄清 | 非必选，如果没有则使用默认的 | `npx skills@latest add mattpocock/skills` |
...
```bash
# 克隆仓库
git clone https://github.com/subfee668/hyper-grill-spec-sp hyper-grill-spec-sp
```

**Claude Code**安装：

```bash
cp -r hyper-grill-spec-sp ~/.claude/skills/hyper-grill-spec-sp
```

**Cursor**安装：

```bash
cp -r hyper-grill-spec-sp .cursor/skills/hyper-grill-spec-sp
```

**Codex CLI**安装：

```bash
cp -r hyper-grill-spec-sp ~/.codex/skills/hyper-grill-spec-sp
```

**OpenCode**安装：

```bash
cp -r hyper-grill-spec-sp ~/.config/opencode/skills/hyper-grill-spec-sp
```
